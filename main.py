...
# 1) Токен только из ENV (Railway → Variables → BOT_TOKEN=...)
RAW_TOKEN = os.getenv("BOT_TOKEN", "")
BOT_TOKEN = (
    RAW_TOKEN.replace("\u00A0", " ").replace("\r", "").replace("\n", "").strip().strip('"').strip("'")
)

# 2) Владелец и админы (жёстко в коде)
OWNER_ID  = 104653853
ADMIN_IDS = [104653853, 1155243378]

# 3) Справочник сотрудников (только эти люди имеют доступ)
#    Заполняй: id: "Фамилия Имя Отчество"
...
def build_csv_today_bytes() -> bytes:
    ...
    Date;Name;ID;Start;End;Duration(h);Weekend;StartReason;EndReason;Comment
    """
    out = io.StringIO()
...
async def main():
    try:
        me = await bot.get_me()  # проверка токена
