...
async def handle_start(message: Message):
    ...
    if is_weekend(now.date()):
        ...
        await message.answer("Начало раньше 08:00. Укажи причину раннего начала:", reply_markup=kb(uid))
    elif now.time() > datetime.time(8, 10):
        await message.answer("Начало позже 08:10. Укажи причину опоздания:", reply_markup=kb(uid))
    else:
...
async def handle_end(message: Message):
    ...
    if now.time() < datetime.time(17, 30):
        await message.answer("Завершение смены раньше 17:30. Укажи причину раннего завершения:", reply_markup=kb(uid))
    elif now.time() > datetime.time(17, 40):
        await message.answer("Смена завершена позже. Укажи причину переработки или напиши "ошибка":", reply_markup=kb(uid))
    else:
...
async def handle_help(message: Message):
    if not ensure_allowed(message): return
    await message.answer(
        "Для фиксации времени начла смены нажми «Начал 🏭». Для фиксации времени завершения смены нажми  «Закончил 🏡».\n"
        "При отклонениях указывай кратко причину по запросу.",
